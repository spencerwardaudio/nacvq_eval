"""Analyze EnCodec token sensitivity to amplitude changes (white noise sweep).

Inputs (under <tokens_root>/white_noise_amp/):
    white_noise_amp_baseline_bw<bw>_tokens.npy
    white_noise_amp_repeat_{1,2}_bw<bw>_tokens.npy
    white_noise_amp_var_<tag>_bw<bw>_tokens.npy   tag ∈ {0, 35, 70, 105, 140}
      tag = dB of attenuation below 0 dBFS  (0 → same as baseline, 140 → silence)

Outputs (under <out_root>/white_noise_amp/):
    heatmap_bw<bw>.png
    mean_flip_vs_var_bw<bw>.png
    stats_bw<bw>.json
    results_bw<bw>.pdf  ← combined PDF for all signals

Usage:
    python analyze_amplitude_test.py [tokens_root] [out_root] [bandwidth]
"""
...
SIGNALS = ["white_noise_amp"]

# Maps tag value (positive dB attenuation) → human-readable x-label
...
def _flip_rate_per_codebook(ref: np.ndarray, other: np.ndarray) -> np.ndarray:
    n = min(ref.shape[1], other.shape[1])  # trim to shorter sequence to handle length mismatches at segment boundaries
...
def analyze_signal(signal: str, tokens_dir: Path, out_dir: Path, bw: str) -> dict | None:
    ...
    var_tags: list[str] = []
    for val, tag, p in variants:  # variants are already sorted by val (attenuation dB)
        et = _load(p)
        if et is None:
            continue
    ...
    stats = {
        ...
        "axis_label": "attenuation (dB below 0 dBFS)",
        "axis_log_x": False,
        "variant_values": var_values,   # positive attenuation magnitudes
...
def _build_heatmap_data(stats: dict) -> tuple[np.ndarray, list[str]]:
    ...
    for tag in stats["variant_tags"]:
        ...
        atten_val = int(float(tag))
        label = _TAG_TO_LABEL.get(atten_val, f"−{tag} dB")
        labels.append(label)
    if not rows:
...
def _heatmap_figure(stats: dict) -> plt.Figure:
    ...
    ax.set_title(f"{stats['signal']} — per-codebook flip rate vs baseline\n"
                 f"(rows = amplitude level, bw={stats['bandwidth']})")
...
def _lineplot_figure(stats: dict) -> plt.Figure:
    n_cb = stats["n_codebooks"]
    var_vals = stats["variant_values"]   # positive attenuation magnitudes
    ...
    # x-axis: positive values represent attenuation, label accordingly
    ax.set_xticks(var_vals)
    ax.set_xticklabels(
        [_TAG_TO_LABEL.get(int(v), f"−{int(v)} dB") for v in var_vals], fontsize=8
    )
    ax.set_xlabel("amplitude (dBFS)")
    ax.set_ylabel("flip rate")
    ax.set_title(f"{stats['signal']} — flip rate vs amplitude (bw={stats['bandwidth']})")
...
def _summary_figure(stats: dict) -> plt.Figure:
    ...
    lines += ["", "Flip rate per amplitude level (mean over all codebooks; cb0 specifically):"]
    lines.append(f"  {'amplitude':>12}   {'mean':>10}   {'cb0':>10}")
    for tag in stats["variant_tags"]:
        atten = int(float(tag))
        label = _TAG_TO_LABEL.get(atten, f"−{tag} dB")
...
def main() -> None:
    tokens_root = (Path(sys.argv[1]) if len(sys.argv) > 1
                   else _PROJ_ROOT / "datasets" / "audio_tokens" / "dsp_amplitude")
    out_root = (Path(sys.argv[2]) if len(sys.argv) > 2
                else _PROJ_ROOT / "datasets" / "analysis" / "dsp_amplitude")
    bw = sys.argv[3] if len(sys.argv) > 3 else "24.0"

    out_root.mkdir(parents=True, exist_ok=True)
    print(f"Amplitude analysis — tokens={tokens_root}  out={out_root}  bw={bw}")
    ...
    for s in all_stats:
        ...
        print(f"  {s['signal']:>20}  DETERMINISTIC={'yes' if det_ok else 'no'}  "
