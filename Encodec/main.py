...
def get_parser():
    ...
    parser.add_argument(
        ...
        help='support encodec_24khz,encodec_48khz,my_encodec,multi_dataset_encodec')
    parser.add_argument(
...
def main(args,model):
    if args.input.suffix.lower() == SUFFIX:
        ...
        out, out_sample_rate = decompress(model, args.input.read_bytes(), device=args.device)
        ...
        # soundfile returns [T] for mono or [T, C] for stereo
        # We need [C, T] format
        if wav.ndim == 1:
            wav = wav.unsqueeze(0)  # [T] -> [1, T] for mono
        elif wav.ndim == 2:
            wav = wav.transpose(0, 1)  # [T, C] -> [C, T] for stereo
