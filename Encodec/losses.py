...
def total_loss(fmap_real, logits_fake, fmap_fake, input_wav, output_wav, sample_rate=24000):
    r"""This function is used to compute the total loss of the encodec generator.
    ...
    for tt1 in range(len(fmap_real)): # len(fmap_real) = 3
        ...
        for tt2 in range(len(fmap_real[tt1])): # len(fmap_real[tt1]) = 5
            ...
            l_feat = l_feat + l1Loss(fmap_real[tt1][tt2], fmap_fake[tt1][tt2]) / (torch.mean(torch.abs(fmap_real[tt1][tt2])) + 1e-8)
...
def disc_loss(logits_real, logits_fake):
    r"""This function is used to compute the loss of the discriminator.
