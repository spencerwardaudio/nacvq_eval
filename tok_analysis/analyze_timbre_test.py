"""Analyze EnCodec token sensitivity to waveform shape (timbre) changes.

Three subdirs — one per amplitude level — each with four waveform shapes.
Baseline = sine wave at that amplitude. Variants: sine(0), saw(1), triangle(2), square(3).

Variant tags are integers 0–3 (floatable); the analyzer maps them to shape names
for axis labels. The lineplot x-axis is categorical (evenly spaced 0–3).

Inputs (under <tokens_root>/timbre_<amp>dB/ for each amplitude):
    timbre_<amp>dB_baseline_bw<bw>_tokens.npy
    timbre_<amp>dB_repeat_{1,2}_bw<bw>_tokens.npy
    timbre_<amp>dB_var_{0,1,2,3}_bw<bw>_tokens.npy

Outputs (under <out_root>/):
    timbre_<amp>dB/heatmap_bw<bw>.png
    timbre_<amp>dB/mean_flip_vs_var_bw<bw>.png
    timbre_<amp>dB/stats_bw<bw>.json
    results_bw<bw>.pdf  ← combined PDF across all three amplitude levels

Usage:
    python analyze_timbre_test.py [tokens_root] [out_root] [bandwidth]
"""
...
SIGNALS = ["timbre_0dB", "timbre_70dB", "timbre_140dB"]  # three amplitude levels from the generator
_SIGNAL_AMP_LABELS = {
    "timbre_0dB": "0 dBFS",
    "timbre_70dB": "−70 dBFS",
    "timbre_140dB": "−140 dBFS (≈ silence)",
}

# Maps integer variant tag → shape name
SHAPE_NAMES: dict[int, str] = {
    0: "sine",      # tag 0 == same shape as baseline — flip rate should be 0
    1: "saw",
    2: "triangle",
    3: "square",
}
...
def _tag_to_shape(tag: str) -> str:
    return SHAPE_NAMES.get(int(float(tag)), tag)
...
def _build_heatmap_data(stats: dict) -> tuple[np.ndarray, list[str]]:
    ...
    for tag in stats["variant_tags"]:
        ...
        labels.append(_tag_to_shape(tag))
    if not rows:
...
def _heatmap_figure(stats: dict) -> plt.Figure:
    ...
    n_cb = matrix.shape[1]
    fig, ax = plt.subplots(figsize=(max(8.0, n_cb * 0.35), 0.55 * len(labels) + 2.0))
    ...
    ax.set_title(f"{stats['signal']} [{stats['amplitude_label']}]\n"
                 f"per-codebook flip rate vs sine baseline  (bw={stats['bandwidth']})")
...
def _lineplot_figure(stats: dict) -> plt.Figure:
    n_cb = stats["n_codebooks"]
    var_vals = stats["variant_values"]   # [0.0, 1.0, 2.0, 3.0]
    ...
    shape_labels = [_tag_to_shape(t) for t in tags]
    ...
    ax.set_xlabel("waveform shape")
    ax.set_ylabel("flip rate vs sine baseline")
    ax.set_title(f"{stats['signal']} [{stats['amplitude_label']}]\n"
                 f"flip rate by waveform shape  (bw={stats['bandwidth']})")
...
def _summary_figure(stats: dict) -> plt.Figure:
    ...
    lines = [
        ...
        f"Baseline: sine 1 kHz — variants differ only in waveform shape",
    ...
    lines += ["", "Flip rate per waveform shape vs sine baseline:"]
    lines.append(f"  {'shape':>10}   {'mean':>10}   {'cb0':>10}")
    for tag in stats["variant_tags"]:
        shape = _tag_to_shape(tag)
        m = stats["mean_flip_rate_by_variant"].get(tag, float("nan"))
        cb0 = stats["flip_rate_by_variant"][tag][0]
        lines.append(f"  {shape:>10}   {m:>10.6f}   {cb0:>10.6f}")
...
def main() -> None:
    tokens_root = (Path(sys.argv[1]) if len(sys.argv) > 1
                   else _PROJ_ROOT / "datasets" / "audio_tokens" / "dsp_timbre")
    out_root = (Path(sys.argv[2]) if len(sys.argv) > 2
                else _PROJ_ROOT / "datasets" / "analysis" / "dsp_timbre")
    bw = sys.argv[3] if len(sys.argv) > 3 else "24.0"

    out_root.mkdir(parents=True, exist_ok=True)
    print(f"Timbre analysis — tokens={tokens_root}  out={out_root}  bw={bw}")
    ...
    for s in all_stats:
        ...
        print(f"  {s['signal']:>15}  DETERMINISTIC={'yes' if det_ok else 'no'}  "
