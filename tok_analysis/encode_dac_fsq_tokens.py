"""Encode sensitivity-test signals with DAC-FSQ and save flat token arrays.

Produces .npy files of shape [1, T] — single FSQ stream per frame — using
the same path convention as the other codec tokenization scripts so that the
existing flip-rate and perplexity analysis functions work unchanged.

DAC-FSQ uses a single Finite Scalar Quantizer (no RVQ residual stack) at 24 kHz.
FSQ levels: [8, 8, 8, 8, 5, 5, 5, 5] → single flat index per frame.

Output path convention:
  datasets/audio_tokens/dac_fsq/{test_name}/{signal}/
      {signal}_baseline_bwFSQ_tokens.npy
      {signal}_var_{tag}_bwFSQ_tokens.npy

Usage:
    python encode_dac_fsq_tokens.py \\
        --checkpoint descript-audio-codec/ckpt/fsd50k_fsq \\
...
def _encode_wav(wav_path: Path, encoder) -> np.ndarray:
    """Return [1, T] int32 token array for one wav file."""
    result = encoder.encode(str(wav_path))
    return result["tokens"]  # [1, T] int32
...
def main() -> None:
    ...
    ap.add_argument("--checkpoint",
                    default=str(_PROJ_ROOT / "descript-audio-codec" / "ckpt" / "fsd50k_fsq"),
                    help="Path to DAC-FSQ checkpoint directory (contains 'latest/' subdir)")
    ...
    ap.add_argument("--out-root",
                    default=str(_DS / "audio_tokens" / "dac_fsq"),
                    help="Root directory for output token .npy files")
    ap.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")
    ap.add_argument("--bw-tag", default="FSQ",
                    help="Bandwidth tag used in output filenames (default: FSQ)")
    ...
    assert checkpoint.exists(), (
        f"DAC-FSQ checkpoint not found: {checkpoint}\n"
        "Train first with train_dac_fsq_fsd50k.sh"
    )
    ...
    encoder = get_codec_encoder("dac_fsq", checkpoint, device=args.device)
    print(f"  FSQ streams: {encoder.n_layers}  (flat single codebook)")
    ...
    print(f"      --codecs encodec q2d2 hificodec dac_fsq \\")
    print(f"      --dac-fsq-bw-tag {args.bw_tag}")
