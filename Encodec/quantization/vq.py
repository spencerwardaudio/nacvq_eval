...
class ResidualVectorQuantizer(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # Q2D2 uses a single quantizer (no residual cascade)
        self.n_q = 1 if use_q2d2 else n_q
    ...
    def encode_pairs(self, x: torch.Tensor) -> torch.Tensor:
        """Q2D2 only: return per-pair indices ``[n_q, n_pairs, B, T]`` for full RVQ analysis.

        Args:
            x: ``[B, D, T]`` encoder output (before quantization).
        Returns:
            ``[n_q, n_pairs, B, T]`` integer indices — n_q stages × n_pairs per stage.
        """
        if not self.use_q2d2:
            raise RuntimeError("encode_pairs is only available when use_q2d2=True")
        
        # Loop through all RVQ layers to get full residual chain
        all_pairs = []
        residual = x
        for layer in self.vq.layers:
            pair_indices = layer.encode_pairs(residual)  # [n_pairs, B, T]
            all_pairs.append(pair_indices)
            # Update residual for next stage
            quantized = layer(residual)[0]
            residual = residual - quantized
        
        # Stack all stages: [n_q, n_pairs, B, T]
        return torch.stack(all_pairs, dim=0)
