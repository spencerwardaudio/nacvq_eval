...
class EncodecModel(nn.Module):
    ...
    def encode(self, x: torch.Tensor) -> tp.List[EncodedFrame]:
        ...
        assert channels > 0 and channels == self.channels
        segment_length = self.segment_length 
    ...
    def forward(self, x: torch.Tensor, return_embeddings: bool = False) -> torch.Tensor:
        ...
        if self.training:
            ...
            decoded = self.decode(codes)[:,:,:x.shape[-1]]
            if return_embeddings:
                return decoded, loss_w, frames, quantized_embeddings
            else:
                return decoded, loss_w, frames
        else:
    ...
    def _get_model(target_bandwidths: tp.List[float],
        ...
        # Use provided n_q or calculate from bandwidth
        if n_q is None:
    ...
    @staticmethod
    def _infer_quantizer_from_state_dict(pre_dic: tp.Dict[str, torch.Tensor]) -> tp.Tuple[bool, str, int]:
        """Infer quantizer variant from checkpoint tensors.

        Returns:
            (use_q2d2, q2d2_grid, q2d2_levels)
        """
        for key, value in pre_dic.items():
            ...
            # Q2D2 codebooks are stored as [grid_points, 2].
            if value.shape[-1] == 2:
                points = int(value.shape[0])
                levels = int(math.isqrt(points))
                if levels * levels != points:
                    # Fallback for non-square point counts; keep robust defaults.
                    levels = max(2, int(round(math.sqrt(points))))
                return True, "rhombic", levels

            break

        return False, "rhombic", 11

    @staticmethod
    def multi_dataset_encodec_model(checkpoint: str, ratios=[8, 5, 4, 2]):
        """Load model trained with multi-dataset training script.
        Matches the config from train_multi_dataset.py:
        - causal=True
        ...
        """
        import os
        assert os.path.exists(checkpoint), f"checkpoint not exists: {checkpoint}"
        print(f"Loading multi-dataset model from: {checkpoint}")
        target_bandwidths = [1.5, 3., 6., 12., 24.]
        sample_rate = 24_000
        channels = 1
        checkpoint_dict = torch.load(checkpoint, map_location='cpu')
        pre_dic = checkpoint_dict['model_state_dict']

        use_q2d2, q2d2_grid, q2d2_levels = EncodecModel._infer_quantizer_from_state_dict(pre_dic)
        if use_q2d2:
            print(f"Detected Q2D2 quantizer checkpoint: grid={q2d2_grid}, levels={q2d2_levels}")

        model = EncodecModel._get_model(
            target_bandwidths, sample_rate, channels,
            causal=True, model_norm='weight_norm', audio_normalize=True,
            segment=None, name='multi_dataset_encodec', ratios=ratios,
            use_q2d2=use_q2d2, q2d2_grid=q2d2_grid, q2d2_levels=q2d2_levels)
        # Try loading with and without key replacement
        try:
            model.load_state_dict(pre_dic, strict=True)
        except RuntimeError:
            # Try with quantizer key replacement
            model.load_state_dict({k.replace('quantizer.model','quantizer.vq'):v for k,v in pre_dic.items()}, strict=False)
        model.eval()
        return model
