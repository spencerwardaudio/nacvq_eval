...
def convert_audio(wav: torch.Tensor, sr: int, target_sr: int, target_channels: int):
    ...
    *shape, channels, length = wav.shape
    if channels == target_channels:
        pass  # No channel conversion needed
    elif target_channels == 1:
        wav = wav.mean(-2, keepdim=True)
    elif channels == 1:
        wav = wav.expand(*shape, target_channels, length)
    elif target_channels == 4 and channels == 2:
        # Stereo -> FOA: place L/R into W (sum) and Y (diff), zero Z and X
        left, right = wav[..., 0:1, :], wav[..., 1:2, :]
        W = (left + right) / 2.0
        Y = (left - right) / 2.0
        Z = torch.zeros_like(W)
        X = torch.zeros_like(W)
        wav = torch.cat([W, Y, Z, X], dim=-2)
    elif target_channels == 2 and channels == 4:
        # FOA -> Stereo: W+Y, W-Y
        W, Y = wav[..., 0:1, :], wav[..., 1:2, :]
        wav = torch.cat([W + Y, W - Y], dim=-2)
    else:
...
def save_audio(wav: torch.Tensor, path: tp.Union[Path, str],
    ...
    # Convert to numpy and save with soundfile (avoids torchcodec dependency)
    # wav is [C, T] format, but soundfile expects [T] (mono) or [T, C] (stereo)
